...
import { Item, Family, Order } from 'src/app/_models';
...
//Pedidos
export class OrderComponent implements OnInit, ownComponent {
  ...
  componentName = { singular: 'Pedidos', plural: 'Pedido' };

  
  elementActive: Order = new Order();
  constructor(
  ...
  beforeSubmit(newElement: Order): boolean {
    //this.dataSource.filter = newElement.Name.trim().toLowerCase();
    return this.dataSource.data.length > 0
  }
  onSubmit() {
    const newElement = new Order();
