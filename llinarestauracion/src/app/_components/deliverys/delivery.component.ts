...
import { Item, Family, Delivery } from 'src/app/_models';
...
//Envios
export class DeliveryComponent implements OnInit, ownComponent {

  @ViewChild('table') table: MatTable<any>;
  elementForm: any;
  elementActive: Delivery = new Delivery();
  itemList: Item[] = [];
  ...
  componentName = { singular: 'Envios', plural: 'Envío' };
  ...
  beforeSubmit(newElement: Delivery): boolean {
    //this.dataSource.filter = newElement.Name.trim().toLowerCase();
    return this.dataSource.data.length > 0
  }
  onSubmit() {
    const newElement = new Delivery();
